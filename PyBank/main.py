...
with open(csvpath, newline="") as csvfile:
    ...
    for row in csvreader:
        ...
        #  calculate change of profit/loss

        if total_months > 1: month_change = int(row[1]) - prev_profit_loss
            
        #  add total monthly change

        total_month_change += month_change
        
        #  set profit/loss value to previous month

        prev_profit_loss = int(row[1])
        
        #  calculate largest increase on profits

        if month_change > greatest_increase:
            greatest_increase = month_change
            greatest_increase_month = row[0]
        
        #  calculate largest decrease on losses

        if month_change < greatest_decrease:
            greatest_decrease = month_change
            greatest_decrease_month = row[0]

#  average change for months 
